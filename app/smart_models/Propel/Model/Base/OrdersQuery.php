...
use \Exception;
use \PDO;
use Propel\Model\Orders as ChildOrders;
 ...
 * @method     ChildOrdersQuery orderByShippingCost($order = Criteria::ASC) Order by the shipping_cost column
 ...
 * @method     ChildOrdersQuery groupByShippingCost() Group by the shipping_cost column
 ...
 * @method     ChildOrders|null findOneByShippingCost(string $shipping_cost) Return the first ChildOrders filtered by the shipping_cost column
 ...
 * @method     ChildOrders requireOneByShippingCost(string $shipping_cost) Return the first ChildOrders filtered by the shipping_cost column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildOrders[]|ObjectCollection findByShippingCost(string $shipping_cost) Return ChildOrders objects filtered by the shipping_cost column
...
abstract class OrdersQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Propel\Model\Base\OrdersQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\Propel\\Model\\Orders', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, user_id, note, first_name, last_name, mobile, email, region, city, zip, address, shipping_first_name, shipping_last_name, shipping_mobile, shipping_email, shipping_region, shipping_city, shipping_zip, shipping_address, shipping_cost, seller_note, status, created_at FROM orders WHERE id = :p0';
        try {
    ...
    public function filterByShippingCost($shippingCost = null, $comparison = null)
    {
        if (is_array($shippingCost)) {
            ...
            if (isset($shippingCost['min'])) {
                $this->addUsingAlias(OrdersTableMap::COL_SHIPPING_COST, $shippingCost['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($shippingCost['max'])) {
                $this->addUsingAlias(OrdersTableMap::COL_SHIPPING_COST, $shippingCost['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(OrdersTableMap::COL_SHIPPING_COST, $shippingCost, $comparison);
    }
