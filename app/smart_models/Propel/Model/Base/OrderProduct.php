...
use \Exception;
use \PDO;
use Propel\Model\OrderProductQuery as ChildOrderProductQuery;
use Propel\Model\Map\OrderProductTableMap;
...
abstract class OrderProduct implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Propel\\Model\\Map\\OrderProductTableMap';
    ...
    /**
     * Initializes internal state of Propel\Model\Base\OrderProduct object.
     */
     ...
     * @return $this|\Propel\Model\OrderProduct The current object (for fluent API support)
     */
    ...
    public function setProductName($v)
    {
        ...
        if ($this->product_name !== $v) {
            $this->product_name = $v;
            $this->modifiedColumns[OrderProductTableMap::COL_PRODUCT_NAME] = true;
        }
    ...
    public function setSubtotal($v)
    {
        ...
        if ($this->subtotal !== $v) {
            $this->subtotal = $v;
            $this->modifiedColumns[OrderProductTableMap::COL_SUBTOTAL] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : OrderProductTableMap::translateFieldName('ProductName', TableMap::TYPE_PHPNAME, $indexType)];
            $this->product_name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : OrderProductTableMap::translateFieldName('Price', TableMap::TYPE_PHPNAME, $indexType)];
            $this->price = (null !== $col) ? (double) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : OrderProductTableMap::translateFieldName('Quantity', TableMap::TYPE_PHPNAME, $indexType)];
            $this->quantity = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : OrderProductTableMap::translateFieldName('Subtotal', TableMap::TYPE_PHPNAME, $indexType)];
            $this->subtotal = (null !== $col) ? (double) $col : null;
            $this->resetModified();
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Propel\\Model\\OrderProduct'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(OrderProductTableMap::COL_PRODUCT_NAME)) {
        ...
        if ($this->isColumnModified(OrderProductTableMap::COL_SUBTOTAL)) {
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\Propel\Model\OrderProduct
     */
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[3], $arr)) {
            $this->setProductName($arr[$keys[3]]);
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Propel\Model\OrderProduct The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(OrderProductTableMap::COL_PRODUCT_NAME)) {
            $criteria->add(OrderProductTableMap::COL_PRODUCT_NAME, $this->product_name);
        }
        if ($this->isColumnModified(OrderProductTableMap::COL_PRICE)) {
            $criteria->add(OrderProductTableMap::COL_PRICE, $this->price);
        }
        if ($this->isColumnModified(OrderProductTableMap::COL_QUANTITY)) {
            $criteria->add(OrderProductTableMap::COL_QUANTITY, $this->quantity);
        }
        if ($this->isColumnModified(OrderProductTableMap::COL_SUBTOTAL)) {
            $criteria->add(OrderProductTableMap::COL_SUBTOTAL, $this->subtotal);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Propel\Model\OrderProduct (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Propel\Model\OrderProduct Clone of current object.
     * @throws PropelException
     */
